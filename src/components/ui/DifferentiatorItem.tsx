...
export function DifferentiatorItem({
  ...
  return (
    ...
    >
      ...
      />
      <div className="shrink-0 grid place-items-center h-10 w-10 rounded-lg bg-[--color-accent-soft] text-(color:--color-accent) transition-all duration-500 group-hover:bg-[--color-accent] group-hover:text-white group-hover:shadow-[0_0_20px_rgba(111,136,164,0.35)]">
        <Icon className="h-4 w-4 transition-transform duration-500 group-hover:rotate-[8deg]" strokeWidth={1.5} />
      </div>
      <div className="flex-1">
        <div className="flex items-baseline gap-3">
          <span className="display text-3xl md:text-4xl text-(color:--color-text-primary)">
            {item.stat}
          </span>
          <span className="text-(color:--color-text-primary) font-medium text-base">
            {item.title}
          </span>
        </div>
        <p className="mt-2 text-sm leading-relaxed text-(color:--color-text-muted)">
          {item.description}
        </p>
      </div>
    </motion.div>
